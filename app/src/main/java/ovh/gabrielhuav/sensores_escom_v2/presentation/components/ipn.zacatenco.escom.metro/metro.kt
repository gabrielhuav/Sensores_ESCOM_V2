...
class metro : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.escom_metro
            )
            ...
            mapView.post {
                // Configurar el mapa
                val normalizedMap = MapMatrixProvider.normalizeMapName(MapMatrixProvider.MAP_METRO)
                mapView.setCurrentMap(normalizedMap, R.drawable.escom_metro)
                ...
                Log.d("metro", "Set map to: $normalizedMap")
            }
        } catch (e: Exception) {
    ...
    private fun checkPositionForMapChange(position: Pair<Int, Int>) {
        ...
        when {
            position.first == 28 && position.second == 15 -> {
                canChangeMap = true
                targetDestination = "general"
                runOnUiThread {
                    Toast.makeText(this, "Presiona B para volver al menu general", Toast.LENGTH_SHORT).show()
                }
            }
    ...
    private fun returnToMainActivity() {
        ...
        val previousPosition = Pair(36, 27) // Posición por defecto si no hay previa
        //val previousPosition = intent.getSerializableExtra("PREVIOUS_POSITION") as? Pair<Int, Int>
        //    ?: Pair(36, 27) // Posición por defecto si no hay previa
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            try {
                ...
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, position, "escom_metro")
                }
    ...
    // Actualiza handleUpdateMessage connor
