...
class BuildingEdificioIA_Alto : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.escom_edificio_ia_planta_alto // Usa la imagen de la cafe de la ESCOM
            )
            ...
            mapView.post {
                ...
                mapView.setCurrentMap(MapMatrixProvider.MAP_EDIFICIO_IA_ALTO, R.drawable.escom_edificio_ia_planta_alto)

                // Configurar el playerManager
                mapView.playerManager.apply {
                    setCurrentMap(MapMatrixProvider.MAP_EDIFICIO_IA_ALTO)
                    localPlayerId = playerName
                    updateLocalPlayerPosition(gameState.playerPosition)
                }

                Log.d(TAG, "Set map to: " + MapMatrixProvider.MAP_EDIFICIO_IA_ALTO)

                // Importante: Enviar un update inmediato para que otros jugadores sepan dónde estamos
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, gameState.playerPosition, MapMatrixProvider.MAP_EDIFICIO_IA_ALTO)
                }
            }
        } catch (e: Exception) {
    ...
    private fun initializeManagers() {
        bluetoothManager = BluetoothManager.getInstance(this, tvBluetoothStatus).apply {
            setCallback(this@BuildingEdificioIA_Alto)
        }


        val onlineServerManager = OnlineServerManager.getInstance(this).apply {
            setListener(this@BuildingEdificioIA_Alto)
        }
    ...
    private fun checkPositionForMapChange(position: Pair<Int, Int>) {
        ...
        when {
            ...
            position.first == 36 && position.second == 18 -> {
                canChangeMap = true
                targetDestination = "bajo"
                runOnUiThread {
                    Toast.makeText(this, "Presiona A para bajar", Toast.LENGTH_SHORT).show()
                }
            }
            else -> {
                canChangeMap = false
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            ...
            if (gameState.isConnected) {
                serverConnectionManager.sendUpdateMessage(playerName, position, "escom_edificio_ia_planta_alto")
            }
    ...
    override fun onPositionReceived(device: BluetoothDevice, x: Int, y: Int) {
        runOnUiThread {
            ...
            mapView.updateRemotePlayerPosition(deviceName, Pair(x, y), MapMatrixProvider.MAP_EDIFICIO_IA_ALTO)
            mapView.invalidate()
        }
    }
                    ...
                    "positions" -> {
                        ...
                        players.keys().forEach { playerId ->
                            if (playerId != playerName) {
                                ...
                                if (map == MapMatrixProvider.MAP_EDIFICIO_IA_ALTO) {
