...
class Salida : AppCompatActivity(),
    ...
    data class GameState(
        ...
        var playerPosition: Pair<Int, Int> = Pair(32, 1),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.escom_estacionamiento
            )
            ...
            mapView.post {
                // Configurar el mapa
                val normalizedMap = MapMatrixProvider.normalizeMapName(MapMatrixProvider.MAP_SALIDA)
                mapView.setCurrentMap(normalizedMap, R.drawable.escom_estacionamiento)
                ...
                Log.d("Salida", "Set map to: $normalizedMap")
            }
        } catch (e: Exception) {
    ...
    private fun initializeComponents(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            gameState.playerPosition = intent.getParcelableExtra("INITIAL_POSITION") ?: Pair(32, 1)
        } else {
    ...
    // Actualiza el método onMapTransitionRequested para manejar la transición al camino
    ...
    private fun starCaminoActivity() {
        val intent = Intent(this, Camino::class.java).apply {
            putExtra("PLAYER_NAME", playerName)
            putExtra("IS_SERVER", gameState.isServer)
            putExtra("INITIAL_POSITION", Pair(3, 38)) // Posición inicial en el salón
        ...
        if (canChangeMap) {
            runOnUiThread {
                when (targetMapId) {
                    ...
                    MapMatrixProvider.MAP_CAMINO-> {
                        Toast.makeText(this, "Presiona A para ir a casa", Toast.LENGTH_SHORT).show()
                    }
                    else -> {
    ...
    private fun startSalidaActivity() {
        val intent = Intent(this, Salida::class.java).apply {
            putExtra("PLAYER_NAME", playerName)
            putExtra("IS_SERVER", gameState.isServer)
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            try {
                ...
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, position, "escom_salida")
                }
