...
class Edificio1Activity : AppCompatActivity(), OnlineServerManager.WebSocketListener, MapView.MapTransitionListener {

    private lateinit var movementManager: MovementManager
    private lateinit var serverConnectionManager: ServerConnectionManager
    private lateinit var mapView: MapView

    // 🎒 Inventario para los puzzles del Edificio 1
    private var tieneManual = false
    private var tieneAcceso = false
    private var sistemaReparado = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView.post {
                mapView.setCurrentMap("edificio1", R.drawable.mapa_edificio3_esime)
                mapView.playerManager.apply {
                    setCurrentMap("edificio1")
            ...
            Log.e("Edificio1", "Error: ${e.message}")
            finish()
        }
    }
    ...
    private fun getRoomName(position: Pair<Int, Int>): String {
        ...
        if (y in 8..17) {
            return when {
                x in 9..12  -> "Laboratorio de Cómputo"
                x in 14..24 -> "Sala de Profesores"
                x in 26..30 -> "Aula Magna"
                x > 31      -> "Archivo"
                else        -> "Muro"
            }
        }
        if (y in 23..31) {
            return when {
                x in 9..14  -> "Coordinación"
                x in 16..21 -> "Sala de Juntas"
                x in 23..29 -> "Bodega"
                x > 30      -> "Escaleras"
                else        -> "Muro"
            }
    ...
    private fun initializeManagers() {
        val onlineServerManager = OnlineServerManager.getInstance(this).apply {
            setListener(this@Edificio1Activity)
        }
            ...
            if (canExitBuilding) {
                ...
                when (room) {
                    "Laboratorio de Cómputo" -> {
                        if (sistemaReparado) {
                            Toast.makeText(this, "💻 El sistema ya está funcionando.", Toast.LENGTH_SHORT).show()
                        } else if (tieneManual && tieneAcceso) {
                            sistemaReparado = true
                            Toast.makeText(this, "✅ ¡Sistemas restaurados! Misión cumplida.", Toast.LENGTH_LONG).show()
                        } else if (tieneManual) {
                            Toast.makeText(this, "🔒 Necesitas las credenciales de acceso. Revisa Coordinación.", Toast.LENGTH_LONG).show()
                        } else {
                            Toast.makeText(this, "💻 Pantallas apagadas. ¿Dónde estará el manual de reinicio?", Toast.LENGTH_LONG).show()
                        }
                    }
                    "Bodega" -> {
                        if (!tieneManual) {
                            tieneManual = true
                            Toast.makeText(this, "📖 ¡Encontraste el Manual de Sistemas en una caja de archivos!", Toast.LENGTH_LONG).show()
                        } else {
                            Toast.makeText(this, "Solo hay cajas viejas aquí.", Toast.LENGTH_SHORT).show()
                        }
                    }
                    "Coordinación" -> {
                        if (!tieneAcceso) {
                            tieneAcceso = true
                            Toast.makeText(this, "🔑 Encuentras una nota pegada: 'Credenciales lab: admin/esime2024'", Toast.LENGTH_LONG).show()
                        } else {
                            Toast.makeText(this, "🔑 Nota: 'Credenciales lab: admin/esime2024'", Toast.LENGTH_SHORT).show()
                        }
                    }
                    "Sala de Profesores", "Aula Magna", "Sala de Juntas", "Archivo" -> {
                        Toast.makeText(this, "Estás en $room. Todo se ve normal.", Toast.LENGTH_SHORT).show()
                    }
                    else -> {
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            ...
            if (::serverConnectionManager.isInitialized) {
                serverConnectionManager.sendUpdateMessage(playerName, position, "edificio1")
            }
    ...
    private fun checkPositionForExit(position: Pair<Int, Int>) {
        ...
        if (canExitBuilding) {
            runOnUiThread {
                Toast.makeText(this, "Presiona A para salir del Edificio 1", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun returnToEsime() {
        val intent = Intent(this, Esime::class.java).apply {
            ...
            putExtra("INITIAL_POSITION", Pair(8, 30))
