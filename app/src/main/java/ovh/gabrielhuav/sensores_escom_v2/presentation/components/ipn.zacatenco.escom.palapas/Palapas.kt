...
class Palapas : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_palapas)

        try {
            // Inicializar el mapView
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.escom_palapas // Usa la imagen de palapas
            )
            ...
            mapView.post {
                ...
                mapView.playerManager.apply {
                    setCurrentMap(MapMatrixProvider.MAP_PALAPAS)
                    localPlayerId = playerName
                    updateLocalPlayerPosition(gameState.playerPosition)
                }

                Log.d(TAG, "Set map to: " + MapMatrixProvider.MAP_PALAPAS)

                // Importante: Enviar un update inmediato para que otros jugadores sepan dónde estamos
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, gameState.playerPosition, MapMatrixProvider.MAP_PALAPAS)
                }
            }
        } catch (e: Exception) {
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            ...
            if (gameState.isConnected) {
                ...
                serverConnectionManager.sendUpdateMessage(playerName, position, MapMatrixProvider.MAP_PALAPAS)
            }
        }
    }
    ...
    override fun onPositionReceived(device: BluetoothDevice, x: Int, y: Int) {
        runOnUiThread {
            ...
            mapView.updateRemotePlayerPosition(deviceName, Pair(x, y), MapMatrixProvider.MAP_PALAPAS)
            mapView.invalidate()
        }
    }
                    ...
                    "positions" -> {
                        ...
                        players.keys().forEach { playerId ->
                            if (playerId != playerName) {
                                ...
                                if (map == MapMatrixProvider.MAP_PALAPAS) {
    ...
    companion object {
        private const val TAG = "Palapas IA"
    }
}