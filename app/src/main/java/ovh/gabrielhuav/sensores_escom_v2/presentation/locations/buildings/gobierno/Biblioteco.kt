...
class Biblioteca : AppCompatActivity(),
    ...
    private fun initializeComponents(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            gameState.playerPosition = (intent.getSerializableExtra("INITIAL_POSITION") as? Pair<Int, Int>)
                ?: MapMatrixProvider.getInitialPositionForMap(MapMatrixProvider.MAP_BIBLIOTECA)
        } else {
    ...
    private fun returnToMainActivity() {
        val previousPosition = intent.getSerializableExtra("PREVIOUS_POSITION") as? Pair<Int, Int>
            ?: MapMatrixProvider.MAIN_TO_BIBLIOTECA
        ...
        runOnUiThread {
            try {
                ...
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, position, MapMatrixProvider.MAP_BIBLIOTECA)
                }
    ...
    override fun onPositionReceived(device: BluetoothDevice, x: Int, y: Int) {
        runOnUiThread {
            ...
            Log.d("Biblioteca", "Recibida posición del dispositivo $deviceName: ($x, $y)")
            mapView.invalidate()
        }
    }
    ...
    companion object {
        private const val TAG = "Biblioteca"
    }
}