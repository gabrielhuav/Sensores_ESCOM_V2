package ovh.gabrielhuav.sensores_escom_v2.presentation.locations.lab
...
class LaboratorioPosgradoActivity : AppCompatActivity(),
    ...
    data class GameState(
        ...
        var playerPosition: Pair<Int, Int> = Pair(5, MapMatrixProvider.MAP_HEIGHT - 5),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_lab_posgrado)

        try {
            // Primero inicializamos el mapView
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.lab_poagrado
            )
            ...
            mapView.post {
                // Configurar el mapa
                val normalizedMap = MapMatrixProvider.normalizeMapName(MapMatrixProvider.MAP_LAB_POSGRADO)
                mapView.setCurrentMap(normalizedMap,  R.drawable.lab_poagrado)
    ...
    private fun initializeComponents(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            gameState.playerPosition =
                (intent.getParcelableExtra("INITIAL_POSITION") ?: Pair(5, MapMatrixProvider.MAP_HEIGHT - 5)) as Pair<Int, Int>
        } else {
    ...
    private fun initializeManagers() {
        bluetoothManager =
            BluetoothManager.Companion.getInstance(this, uiManager.tvBluetoothStatus).apply {
                setCallback(this@LaboratorioPosgradoActivity)
            }
        ...
        val onlineServerManager = OnlineServerManager.Companion.getInstance(this).apply {
            setListener(this@LaboratorioPosgradoActivity)
        }
    ...
    override fun onMapTransitionRequested(targetMap: String, initialPosition: Pair<Int, Int>) {
        when {
            (targetMap == MapMatrixProvider.MAP_MAIN) -> {
            ...
            runOnUiThread {
                when (targetMapId) {
                    MapMatrixProvider.MAP_MAIN -> {
                        Toast.makeText(
                            this,
                            "Presiona A para volver al mapa principal",
                            Toast.LENGTH_SHORT
                        ).show()
                    }

                    else -> {
                        Toast.makeText(this, "Presiona A para interactuar", Toast.LENGTH_SHORT)
                            .show()
                    }
                }
            }
