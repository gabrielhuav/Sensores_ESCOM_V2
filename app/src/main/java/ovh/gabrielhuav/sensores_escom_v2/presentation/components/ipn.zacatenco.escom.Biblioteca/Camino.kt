...
class Camino : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.camino_casa // Usa la imagen del salón 2009
            )
            ...
            mapView.post {
                ...
                mapView.setCurrentMap(MapMatrixProvider.MAP_CAMINO, R.drawable.camino_casa)

                // Configurar el playerManager
                mapView.playerManager.apply {
                    setCurrentMap(MapMatrixProvider.MAP_CAMINO)
                    localPlayerId = playerName
                    updateLocalPlayerPosition(gameState.playerPosition)
                }

                Log.d(TAG, "Set map to: " + MapMatrixProvider.MAP_CAMINO)

                // Importante: Enviar un update inmediato para que otros jugadores sepan dónde estamos
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, gameState.playerPosition, MapMatrixProvider.MAP_CAMINO)
                }
            }
        } catch (e: Exception) {
    ...
    override fun onPositionReceived(device: BluetoothDevice, x: Int, y: Int) {
        runOnUiThread {
            ...
            mapView.updateRemotePlayerPosition(deviceName, Pair(x, y), MapMatrixProvider.MAP_CAMINO)
            mapView.invalidate()
        }
    }
                        ...
                        players.keys().forEach { playerId ->
                            if (playerId != playerName) {
                                ...
                                gameState.remotePlayerPositions = gameState.remotePlayerPositions +
                                        (playerId to Biblioteca.GameState.PlayerInfo(position, normalizedMap))

                                // Obtener el mapa actual normalizado para comparar
                                val currentMap = MapMatrixProvider.normalizeMapName(MapMatrixProvider.MAP_CAMINO)
