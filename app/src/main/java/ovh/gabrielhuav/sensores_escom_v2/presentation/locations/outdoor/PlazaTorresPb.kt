...
class PlazaTorresPb : AppCompatActivity(),
    ...
    private fun initializeComponents(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            // Usamos la posición inicial recibida de Zacatenco.
            gameState.playerPosition =
                (intent.getSerializableExtra("INITIAL_POSITION") as? Pair<Int, Int>)
                    ?: Pair(2, 21) // ¡¡CAMBIO AQUÍ!! Usamos una posición de respaldo más lógica para la entrada
        } else {
    ...
    private fun checkPositionForMapChange(position: Pair<Int, Int>) {
        // ¡¡CAMBIO AQUÍ!! Punto de entrada/salida. REEMPLAZA (2, 21) CON LA COORDENADA QUE OBTENGAS DEL LOGCAT
        val entryPoint = Pair(2, 21)
    ...
    private fun returnToZacatencoActivity() {
        ...
        val previousPosition = intent.getSerializableExtra("PREVIOUS_POSITION") as? Pair<Int, Int>
            ?: Pair(13, 11) // ¡¡CAMBIO AQUÍ!! Usamos la coordenada REAL de Zacatenco (14,10) como respaldo
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            try {
                Log.d(TAG, "Nueva posición en Plaza Torres: X=${position.first}, Y=${position.second}")
    ...
    companion object {
        // ¡¡CAMBIO AQUÍ!! Un TAG específico para esta clase, para filtrar mejor los logs
        private const val TAG = "PlazaTorresDebug"
    }
}