...
class Edificio4Activity : AppCompatActivity(), OnlineServerManager.WebSocketListener, MapView.MapTransitionListener {

    private lateinit var movementManager: MovementManager
    private lateinit var serverConnectionManager: ServerConnectionManager
    private lateinit var mapView: MapView

    // 🎒 Inventario para los puzzles del Edificio 4
    private var tieneChip = false
    private var tieneDiagrama = false
    private var circuitoReparado = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView.post {
                mapView.setCurrentMap("edificio4", R.drawable.mapa_edificio3_esime)
                mapView.playerManager.apply {
                    setCurrentMap("edificio4")
            ...
            Log.e("Edificio4", "Error: ${e.message}")
            finish()
        }
    }
    ...
    private fun getRoomName(position: Pair<Int, Int>): String {
        ...
        if (y in 8..17) {
            return when {
                x in 9..12  -> "Lab. Electrónica"
                x in 14..24 -> "Sala de Prototipado"
                x in 26..30 -> "Aula de Circuitos"
                x > 31      -> "Cuarto de Equipos"
                else        -> "Muro"
            }
        }
        if (y in 23..31) {
            return when {
                x in 9..14  -> "Depósito de Componentes"
                x in 16..21 -> "Lab. de Pruebas"
                x in 23..29 -> "Sala de Servidores"
                x > 30      -> "Escaleras"
                else        -> "Muro"
            }
        }
    ...
    private fun initializeManagers() {
        val onlineServerManager = OnlineServerManager.getInstance(this).apply {
            setListener(this@Edificio4Activity)
        }
        ...
        buttonA.setOnClickListener {
            if (canExitBuilding) {
                ...
                when (room) {
                    "Lab. Electrónica" -> {
                        if (circuitoReparado) {
                            Toast.makeText(this, "⚡ Los circuitos funcionan perfectamente.", Toast.LENGTH_SHORT).show()
                        } else if (tieneChip && tieneDiagrama) {
                            circuitoReparado = true
                            Toast.makeText(this, "✅ ¡Circuito principal reparado! Todos los sistemas en línea.", Toast.LENGTH_LONG).show()
                        } else if (tieneChip) {
                            Toast.makeText(this, "🔌 Tienes el chip pero necesitas el diagrama para instalarlo.", Toast.LENGTH_LONG).show()
                        } else {
                            Toast.makeText(this, "⚡ Panel quemado. Se necesita un chip de repuesto.", Toast.LENGTH_LONG).show()
                        }
                    }
                    "Depósito de Componentes" -> {
                        if (!tieneChip) {
                            tieneChip = true
                            Toast.makeText(this, "🔲 ¡Encontraste un microchip ATmega328 en el inventario!", Toast.LENGTH_LONG).show()
                        } else {
                            Toast.makeText(this, "Solo quedan resistencias y capacitores.", Toast.LENGTH_SHORT).show()
                        }
                    }
                    "Sala de Servidores" -> {
                        if (!tieneDiagrama) {
                            tieneDiagrama = true
                            Toast.makeText(this, "📋 ¡Descargaste el diagrama de circuitos desde el servidor!", Toast.LENGTH_LONG).show()
                        } else {
                            Toast.makeText(this, "📋 El diagrama ya está en tu memoria.", Toast.LENGTH_SHORT).show()
                        }
                    }
                    "Sala de Prototipado", "Aula de Circuitos", "Cuarto de Equipos", "Lab. de Pruebas" -> {
                        Toast.makeText(this, "Estás en $room. Todo en orden.", Toast.LENGTH_SHORT).show()
                    }
                    else -> {
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            ...
            if (::serverConnectionManager.isInitialized) {
                serverConnectionManager.sendUpdateMessage(playerName, position, "edificio4")
            }
    ...
    private fun checkPositionForExit(position: Pair<Int, Int>) {
        ...
        if (canExitBuilding) {
            runOnUiThread {
                Toast.makeText(this, "Presiona A para salir del Edificio 4", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun returnToEsime() {
        val intent = Intent(this, Esime::class.java).apply {
            ...
            putExtra("INITIAL_POSITION", Pair(8, 11))
