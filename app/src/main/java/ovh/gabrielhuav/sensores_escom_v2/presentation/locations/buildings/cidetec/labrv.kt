...
class labrv : AppCompatActivity(),
    ...
    data class GameState(
        ...
        var playerPosition: Pair<Int, Int> = Pair(7, 30),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.laboratoriorv
            )
            ...
            mapView.post {
                // Configurar el mapa
                val normalizedMap = MapMatrixProvider.Companion.normalizeMapName(MapMatrixProvider.Companion.MAP_LABRV)
                mapView.setCurrentMap(normalizedMap, R.drawable.laboratoriorv)
                ...
                Log.d("labrv", "Set map to: $normalizedMap")
            }
        } catch (e: Exception) {
    ...
    override fun onMapTransitionRequested(targetMap: String, initialPosition: Pair<Int, Int>) {
        when (targetMap) {
            MapMatrixProvider.Companion.MAP_CIDETEC -> {
                // Transición al mapa principal
                returnToCidetecActivity()
            }
    ...
    private fun checkPositionForMapChange(position: Pair<Int, Int>) {
        ...
        when {
            position.first == 7 && position.second == 30 -> {
                canChangeMap = true
                targetDestination = "cidetec"
                runOnUiThread {
                    Toast.makeText(this, "Presiona A para regresar a Cidetec", Toast.LENGTH_SHORT)
                        .show()
                }
            }
            ...
            buttonA.setOnClickListener {
                if (canChangeMap) {
                    when (targetDestination) {
                        "cidetec" -> returnToCidetecActivity()
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            try {
                ...
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, position, "labrv")
                }
