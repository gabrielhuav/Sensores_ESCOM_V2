...
class Avenida : AppCompatActivity(),
        ...
        try {
            ...
            mapView.post {
                ...
                mapView.playerManager.apply {
                    setCurrentMap(MapMatrixProvider.MAP_AVENIDA)
                    localPlayerId = playerName
                    updateLocalPlayerPosition(gameState.playerPosition)
                }

                Log.d(TAG, "Set map to: " + MapMatrixProvider.MAP_AVENIDA)

                // Importante: Enviar un update inmediato para que otros jugadores sepan dónde estamos
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, gameState.playerPosition, MapMatrixProvider.MAP_AVENIDA)
                }
            }
        } catch (e: Exception) {
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            ...
            if (gameState.isConnected) {
                ...
                serverConnectionManager.sendUpdateMessage(playerName, position, MapMatrixProvider.MAP_AVENIDA)
            }
        }
    }
    ...
    override fun onPositionReceived(device: BluetoothDevice, x: Int, y: Int) {
        runOnUiThread {
            ...
            mapView.updateRemotePlayerPosition(deviceName, Pair(x, y), MapMatrixProvider.MAP_AVENIDA)
            mapView.invalidate()
        }
    }
                    ...
                    "positions" -> {
                        ...
                        players.keys().forEach { playerId ->
                            if (playerId != playerName) {
                                ...
                                if (map == MapMatrixProvider.MAP_AVENIDA) {
    ...
    companion object {
        private const val TAG = "Estacionamiento"
    }
}