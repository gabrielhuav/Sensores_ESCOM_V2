package ovh.gabrielhuav.sensores_escom_v2.presentation.locations.buildings.plaza
...
class PlazaVistaNorte : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_plaza_vista_norte)

        try {
            // Inicializar el mapView
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.plaza_vista_norte // Usa la imagen de la plaza
            )
            ...
            mapView.post {
                ...
                mapView.setCurrentMap(MapMatrixProvider.MAP_PLAZA_VISTA_NORTE, R.drawable.plaza_vista_norte)

                // Configurar el playerManager
                mapView.playerManager.apply {
                    setCurrentMap(MapMatrixProvider.MAP_PLAZA_VISTA_NORTE)
                    localPlayerId = playerName
                    updateLocalPlayerPosition(gameState.playerPosition)
                }

                Log.d(TAG, "Set map to: " + MapMatrixProvider.MAP_PLAZA_VISTA_NORTE)

                // Importante: Enviar un update inmediato para que otros jugadores sepan dónde estamos
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, gameState.playerPosition, MapMatrixProvider.MAP_PLAZA_VISTA_NORTE)
                }
            }
        } catch (e: Exception) {
            ...
            runOnUiThread {
                ...
                if (success) {
                    ...
                    serverConnectionManager.sendUpdateMessage(
                        ...
                        MapMatrixProvider.MAP_PLAZA_VISTA_NORTE
                    )

                    // Solicitar actualizaciones de posición
                    serverConnectionManager.onlineServerManager.requestPositionsUpdate()

                    updateBluetoothStatus("Conectado al servidor online - Plaza Vista Norte")
                } else {
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            ...
            if (gameState.isConnected) {
                ...
                serverConnectionManager.sendUpdateMessage(playerName, position, MapMatrixProvider.MAP_PLAZA_VISTA_NORTE)
            }
        }
    }
    ...
    override fun onPositionReceived(device: BluetoothDevice, x: Int, y: Int) {
        runOnUiThread {
            ...
            mapView.updateRemotePlayerPosition(deviceName, Pair(x, y), MapMatrixProvider.MAP_PLAZA_VISTA_NORTE)
            mapView.invalidate()
        }
    }
                ...
                when (jsonObject.getString("type")) {
                    ...
                    "update" -> {
                        ...
                        if (playerId != playerName) {
                            ...
                            // Obtener el mapa actual normalizado para comparar
                            val currentMap = MapMatrixProvider.normalizeMapName(MapMatrixProvider.MAP_PLAZA_VISTA_NORTE)
    ...
    companion object {
        private const val TAG = "PlazaVistaNorte"
    }
}
